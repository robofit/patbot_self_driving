...
#include<patbotsd_map_maker_laser_asus_directions.h>
...
void Patbot_Map_Maker::Scan_Hokuyo_callback(const sensor_msgs::LaserScan::ConstPtr& msg){
   ...
   //Get [x,y,theta] values of the max element of the array  
    max_range_element_x  =  laser_range_vector_[index_max_range_element] *cos(1.5708-(index_max_range_element*inc)); 
    max_range_element_y  =  laser_range_vector_[index_max_range_element] *sin(1.5708-(index_max_range_element*inc)); 
   ...
   patbot_right_frees_pace_.clear();
   patbot_right_frees_pace_.resize(98);
   for(int i = 0; i < 98; i++){
      double vectorin = laser_range_vector_[621+i];
      patbot_right_frees_pace_[i] = vectorin;
   }


   //Store all ranges on the front-right side of the robot in a vector
   patbot_front_right_frees_pace_.clear();
   patbot_front_right_frees_pace_.resize(98);
   for(int i = 0; i < 98; i++){
      double vectorin = laser_range_vector_[491+i];
      patbot_front_right_frees_pace_[i] = vectorin;
   }



   //Store all ranges on the left side of the robot in a vector
   patbot_left_frees_pace_.clear();
   patbot_left_frees_pace_.resize(98);
   for(int i = 0; i < 98; i++){
      double vectorin = laser_range_vector_[i];
      patbot_left_frees_pace_[i] = vectorin;
   }


   //Store all ranges on the front-left side of the robot in a vector
   patbot_front_left_frees_pace_.clear();
   patbot_front_left_frees_pace_.resize(98);
   for(int i = 0; i < 98; i++){
      double vectorin = laser_range_vector_[131+i];
      patbot_front_left_frees_pace_[i] = vectorin;
}
   ...
   for(int i = 0; i < 98; i++){
      if(patbot_right_frees_pace_[i] > 2.0){
         wall_count_right_++;
      }
   }
   for(int i = 0; i < 98; i++){
      if(patbot_front_right_frees_pace_[i] > 2.0){
         wall_count_front_right_++;
      }
   }
   for(int i = 0; i < 98; i++){
      if(patbot_left_frees_pace_[i] > 2.0){
         wall_count_left_++;
      }
   }
   for(int i = 0; i < 98; i++){
      if(patbot_front_left_frees_pace_[i] > 2.0){
   ...
   if((front_right_ == true) && (front_left_ == true)){
      front_right_ = true;
      right_ = false;
      front_left_ = false;
      left_ = false;
   ...
   if(obstacle_flag_ == true){
      for(int i = 0; i < free_space; i++){
         if(patbot_front_frees_pace_[i] < 1.5){ 
...
int main(int argc, char** argv){
   ros::init(argc, argv, "patbotsd_map_maker_laser_asus_directions");
   Patbot_Map_Maker patbot_map_maker;
